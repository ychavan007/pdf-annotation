...
#define PDFVIEW_LOG_TAG "cx.pdf.android.pdfview"
...
Java_cx_pdf_android_pdfview_PDF_parseFile(
        JNIEnv *env,
        jobject jthis,
        jstring file_name,
        jint box_type) {
 ...
 */
JNIEXPORT void JNICALL
Java_cx_pdf_android_pdfview_PDF_parseFileDescriptor(
        JNIEnv *env,
        jobject jthis,
 ...
 */
JNIEXPORT jint JNICALL
Java_cx_pdf_android_pdfview_PDF_getPageCount(
		JNIEnv *env,
		jobject this) {
	pdf_t *pdf = NULL;
...
Java_cx_pdf_android_pdfview_PDF_renderPage(
        JNIEnv *env,
        jobject this,
        jint pageno,
        jint zoom,
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "cx.pdf.android.pdfview", "jni renderPage(pageno: %d, zoom: %d, left: %d, top: %d, width: %d, height: %d) start",
            (int)pageno, (int)zoom,
            (int)left, (int)top,
            (int)width, (int)height);
...
JNIEXPORT jint JNICALL
Java_cx_pdf_android_pdfview_PDF_getPageSize(
        JNIEnv *env,
        jobject this,
        jint pageno,
        jobject size) {
    ...
    pdf = get_pdf_from_this(env, this);
    if (pdf == NULL) {
        __android_log_print(ANDROID_LOG_ERROR, "cx.pdf.android.pdfview", "this.pdf is null");
        return 1;
    }

    error = get_page_size(pdf, pageno, &width, &height);
    if (error != 0) {
        __android_log_print(ANDROID_LOG_ERROR, "cx.pdf.android.pdfview", "get_page_size error: %d", (int)error);
        /*
        __android_log_print(ANDROID_LOG_ERROR, "cx.pdf.android.pdfview", "fitz error is:\n%s", fz_errorbuf);
        */
        return 2;
    }
 ...
 */
JNIEXPORT void JNICALL
Java_cx_pdf_android_pdfview_PDF_freeMemory(
        JNIEnv *env,
        jobject this) {
    pdf_t *pdf = NULL;
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "cx.pdf.android.pdfview", "jni freeMemory()");
...
#if 0
JNIEXPORT void JNICALL
Java_cx_pdf_android_pdfview_PDF_export(
        JNIEnv *env,
        jobject this) {
    pdf_t *pdf = NULL;
...
Java_cx_pdf_android_pdfview_PDF_find(
        JNIEnv *env,
        jobject this,
        jstring text,
        jint pageno) {
...
jobject create_find_result(JNIEnv *env) {
    ...
    findResultClass = (*env)->FindClass(env, "cx/pdf/android/lib/pagesview/FindResult");
...
pdf_t* get_pdf_from_this(JNIEnv *env, jobject this) {
    ...
    if (! field_is_cached) {
        ...
        field_is_cached = 1;
        __android_log_print(ANDROID_LOG_DEBUG, "cx.pdf.android.pdfview", "cached pdf_ptr field id %d", (int)field_id);
    }
...
int get_descriptor_from_file_descriptor(JNIEnv *env, jobject this) {
    ...
    if (!is_cached) {
        ...
        is_cached = 1;
        __android_log_print(ANDROID_LOG_DEBUG, "cx.pdf.android.pdfview", "cached descriptor field id %d", (int)field_id);
    }
    __android_log_print(ANDROID_LOG_DEBUG, "cx.pdf.android.pdfview", "will get descriptor field...");
...
void get_size(JNIEnv *env, jobject size, int *width, int *height) {
    ...
    if (! fields_are_cached) {
        ...
        fields_are_cached = 1;
        __android_log_print(ANDROID_LOG_DEBUG, "cx.pdf.android.pdfview", "cached Size fields");
    }
...
pdf_t* parse_pdf_bytes(unsigned char *bytes, size_t len, jstring box_name) {
    ...
    if (error) {
        __android_log_print(ANDROID_LOG_ERROR, "cx.pdf.android.pdfview", "got err from pdf_loadxref_mem: %d", (int)error);
        __android_log_print(ANDROID_LOG_ERROR, "cx.pdf.android.pdfview", "fz errors:\n%s", fz_errorbuf);
        /* TODO: free resources */
        return NULL;
    }
    ...
    if (pdf_needspassword(pdf->xref)) {
        ...
        if (!authenticated) {
            ...
            __android_log_print(ANDROID_LOG_ERROR, "cx.pdf.android.pdfview", "failed to authenticate with empty password");
            return NULL;
        }
    }
...
pdf_t* parse_pdf_file(const char *filename, int fileno) {
    ...
    */
    ...
    obj = fz_dict_gets(pageobj, "Annots");
    	if (obj)
    	{
    		pdf->annot = pdf_load_annots(NULL, pdf->xref, obj);
    	}*/
    /** !!! */
...
pdf_page* get_page(pdf_t *pdf, int pageno) {
    ...
    if (!pdf->pages[pageno]) {
        ...
        if (error) {
            __android_log_print(ANDROID_LOG_ERROR, "cx.pdf.android.pdfview", "pdf_loadpage -> %d", (int)error);
            /* __android_log_print(ANDROID_LOG_ERROR, "cx.pdf.android.pdfview", "fitz error is:\n%s", fz_errorbuf); */
            return NULL;
        }
...
void pdf_android_loghandler(const char *m) {
    __android_log_print(ANDROID_LOG_DEBUG, "cx.pdf.android.pdfview.mupdf", m);
}
